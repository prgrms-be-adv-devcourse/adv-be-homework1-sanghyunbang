...
---
### 3. 코어가 인프라에 의존하는 문제 해결하기 : 의존성 역전 원칙 구현(DIP)

![리펙토링3](./images/refactor3-problem.png)
** 위의 이미지에서 보듯이 orderService가 직접 orderJpaRepository를 의존하는 상황**

![리펙토링3](./images/refactor3.png)

**DIP원칙을 적용해서 고수준 모듈인 비즈니스 로직(orderService)가 저수준 모듈인 (orderJpaRepository)에 의존하지 않고
둘 다 추상화된 port 인터페이스에 의존하도록 구조를 바꾸기**
